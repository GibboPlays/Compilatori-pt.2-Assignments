...
struct TestPass: PassInfoMixin<TestPass> {
  ...
  void runControl(std::vector<Instruction*> &toErase, Instruction &Inst, int idxreg, ConstantInt*sumdiff)
  {
    ...
    for (auto Iter = Inst.user_begin(); Iter != Inst.user_end(); ++Iter) {
      ...
      //l'istruzione che cerco è add (se Instr è sub) o sub (se Instr è add)
      if ((Inst.getOpcode() == Instruction::Sub && InstAdd.getOpcode() == Instruction::Add) ||
          (Inst.getOpcode() == Instruction::Add && InstAdd.getOpcode() == Instruction::Sub))
      {
